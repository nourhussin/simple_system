...
task CHECK_RD ;
 ...
 begin
	...
	if(gener_out == expec_out) 
		begin
			$display("Read Operation is succeeded with configurations PARITY_ENABLE=%d PARITY_TYPE=%d  PRESCALE=%d  ",DUT.UART_Config[0],DUT.UART_Config[1],DUT.UART_Config[7:2]);
		end
	else
		begin
...
task CHECK_ALU ;
 ...
 begin
	...
	case (FUNCTION[3:0]) 
     4'b0000: begin
               ALU_OUT_RESULT = DUT.U_RegFile.regfile[0] + DUT.U_RegFile.regfile[1];
              end
     4'b0001: begin
               ALU_OUT_RESULT = DUT.U_RegFile.regfile[0] - DUT.U_RegFile.regfile[1];
              end
     ...
     4'b1010: begin
              if (DUT.U_RegFile.regfile[0] == DUT.U_RegFile.regfile[1])
                 ALU_OUT_RESULT = 'b1;
              else
                 ALU_OUT_RESULT = 'b0;
              end
     ...
     4'b1101: begin
               ALU_OUT_RESULT = DUT.U_RegFile.regfile[0]>>1;
              end
     4'b1110: begin 
               ALU_OUT_RESULT = DUT.U_RegFile.regfile[0]<<1;
              end
	...
	if(gener_byte0 == expec_byte0 && gener_byte1 == expec_byte1) 
		begin
			$display("ALU Operation is succeeded with configurations PARITY_ENABLE=%d PARITY_TYPE=%d  PRESCALE=%d  ",DUT.UART_Config[0],DUT.UART_Config[1],DUT.UART_Config[7:2]);
		end
	else
		begin
